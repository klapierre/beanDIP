...
color_chewing_facet<-
  ...
  scale_color_manual(values=c("#998ec3","#f1a340"),labels=c("Treated","Untreated"))+
...
color_biomass_facet<-
  ...
  scale_color_manual(values=c("#f1a340","#998ec3"),labels=c("Untreated","Treated"))+
...
## Stand density
```{r}
dens2019 <- read.csv('C:/Users/kmcgurri/Dropbox/bean_dip_2018-2024/field trials/data/clean_data/clean_damage_2019.csv') %>% 
  mutate(year=2019,
         variety_name=case_when(variety %in% c(67,27) ~ 'AG38X8'),
         seed_coat=case_when(variety==67 ~ 'untrt',
                             variety==27 ~ 'trt')) %>%
  select(year,sampling.round,site,variety_name,seed_coat,plot,row,density)%>%
  filter(sampling.round==1)

dens2020 <- read.csv('C:/Users/kmcgurri/Dropbox/bean_dip_2018-2024/field trials/data/clean_data/clean_damage_2020.csv') %>% 
  mutate(year=2020,
         variety_name=case_when(variety %in% c(2,5) ~ 'AG38X8'),
         seed_coat=case_when(variety==2 ~ 'untrt',
                             variety==5 ~ 'trt')) %>%
...
dens2021 <- read.csv('C:/Users/kmcgurri/Dropbox/bean_dip_2018-2024/field trials/data/clean_data/clean_damage_2021.csv') %>% 
  mutate(year=2021,
         variety_name=case_when(variety %in% c(19,59) ~ 'AG38X8'),
         seed_coat=case_when(variety == 19 ~ 'untrt',
                             variety == 59 ~ 'trt')) %>%
...
density <- rbind(dens2019, dens2020, dens2021) %>% #bind all data together
  group_by(year, site, variety_name, seed_coat, plot) %>% # average rows a and b to get one value per plot
  summarise(avg_plot_density= mean(density, na.rm = TRUE),
            plants_per_foot=avg_plot_density/4)

# summary table for mean and SE
density_siteAvgDf<-density %>%
  group_by(year,seed_coat) %>%
  summarise(site_avg_stand_density=mean(plants_per_foot,na.rm=T),
            se_stand_density=sd(plants_per_foot,na.rm = T)/sqrt(n()))

ggplot(density_siteAvgDf,aes(x=rev(seed_coat),y=site_avg_stand_density,
        shape=rev(seed_coat),linetype=rev(seed_coat),color=rev(seed_coat)))+
         geom_line()+ geom_point(size=3)+ facet_wrap(~year,nrow=1)+
  scale_linetype_manual(values=c("dashed","solid"),labels=c("Untreated","Treated"))+
  scale_shape_manual(values = c(19,17),labels=c("Untreated","Treated"))+
  scale_color_manual(values=c("#f1a340","#998ec3"),labels=c("Untreated","Treated"))+
         geom_errorbar(aes(ymin=site_avg_stand_density-se_stand_density,
                           ymax=site_avg_stand_density+se_stand_density),width=0.2)+
  labs(y="Stand Density (plants/ft)\nAvg ± SE",x="")+
  scale_x_discrete(labels=c("Untreated","Treated")) +
  theme_bw() + theme(panel.grid.minor = element_blank(),
                     strip.text = element_text(face="bold",size=rel(1.5)),legend.position = "none")


ggplot(data=filter(density,year==2019),aes(x=plants_per_foot))+geom_histogram(binwidth = 1)
dens_2019_mod<-lmer(plants_per_foot~seed_coat+(1|site),data=filter(density,year==2019))
anova(dens_2019_mod)
simulationOutput<-simulateResiduals(fittedModel = dens_2019_mod)
plot(simulationOutput)

ggplot(data=filter(density,year==2020),aes(x=plants_per_foot))+geom_histogram(binwidth = 1)
dens_2020_mod<-lmer(plants_per_foot~seed_coat+(1|site),data=filter(density,year==2020))
anova(dens_2020_mod)
simulationOutput<-simulateResiduals(fittedModel = dens_2020_mod)
plot(simulationOutput)

ggplot(data=filter(density,year==2021),aes(x=plants_per_foot))+geom_histogram(binwidth = 1)
dens_2021_mod<-lmer(plants_per_foot~seed_coat+(1|site),data=filter(density,year==2021))
anova(dens_2021_mod)
simulationOutput<-simulateResiduals(fittedModel = dens_2021_mod)
plot(simulationOutput)
