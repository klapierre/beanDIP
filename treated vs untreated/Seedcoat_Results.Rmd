...
```{r echo=TRUE}
AG_chew_mod<-lmer(chew_pct~year*sampling.round*seed_coat+(1|site),data=AG_damage)
...
```{r echo=TRUE}
AG_stip_mod<-lmer(stip_pct~year*sampling.round*seed_coat+(1|site),data=AG_damage)
...
```
Note: how to handle zero-inflation? 

```{r echo=TRUE}
AG_chewer_mod<-glmer(count~year*sampling_round*seed_coat+(1|site),family="poisson",
                    data=AG_insects %>% filter(functional_group=="chewing"))

hist(AG_insects$count[AG_insects$functional_group=="chewing"])
summary(as.factor(AG_insects$count[AG_insects$functional_group=="chewing"]))
...
```
```{r echo=TRUE}
AG_sucker_mod<-lmer(count~year*sampling_round*seed_coat+(1|site),
...
```{r echo=TRUE}
AG_biomass_mod<-lmer(plant_biomass_g~year*seed_treat+(1|site),data=AG_biomass)
...
```{r echo=TRUE}
AG_yield_mod<-lmer(plot_yield_buac~year*seed_treat+(1|site),data=AG_biomass)
...
```{r echo=TRUE}
SH_chew_mod<-lmer(chew_pct~year*sampling.round*seed_coat+(1|site),data=SH_damage)
...
```{r echo=TRUE}
SH_stip_mod<-lmer(stip_pct~year*sampling.round*seed_coat+(1|site),data=SH_damage)
...
Note: angry model. 
```{r echo=TRUE}
SH_chewer_mod<-lmer(count~sampling_round*seed_coat+(1|site),
...
```{r echo=TRUE}
SH_sucker_mod<-lmer(count~sampling_round*seed_coat+(1|site),
...
```{r echo=TRUE}
SH_biomass_mod<-lmer(plant_biomass_g~seed_treat+(1|site),data=SH_biomass)
...
```{r echo=TRUE}
SH_yield_mod<-lmer(plot_yield_buac~seed_treat+(1|site),data=SH_biomass)
