...
# AG_chewer_2021_mod<-lmer(count~seed_coat*as.factor(sampling_round)+(1|site),
#                     data=filter(AG_chewer, year==2021))
# AG_chewer_2021_mod<-glmer.nb(count~seed_coat*as.factor(sampling_round)+(1|site),
#                     data=filter(AG_chewer, year==2021))
...
# code from Karin
library(glmmTMB)
fit_zinbinom1comp<-glmmTMB(count~seed_coat*as.factor(sampling_round)+(1|site), data=filter(AG_chewer, year==2021), ziformula=~., family=nbinom1)
summary(fit_zinbinom1comp)
Anova(fit_zinbinom1comp)
# ##DHARMA CHECK###
library(DHARMa)
simulationOutput <- simulateResiduals(fittedModel = fit_zinbinom1comp)
...
AG_insect_2019_mod<-lmer(count~seed_coat*as.factor(sampling_round)+(1|site),
                    data=filter(AG_insects, year == 2019)) # fit is singular
...
SH_chewer_2021_mod<-lmer(count~seed_coat*as.factor(sampling_round)+(1|site),
                    data=filter(SH_insects,functional_group=="chewing")) # fit is singular
