...
emmeans(chewer_2019_mod2,specs = pairwise ~ sampling_round) # 3 higher than others
...
emmeans(chewer_2020_mod2,specs = pairwise ~ sampling_round) # 5 higher than 1, 2, and 3
...
# model has singular fit, so tried running 3 other ways (see below): variety as fixed effect instead of random, with poisson distribution, and with outlier removed. all got same result, only samp.round signif, so kept simplest model
anova(chewer_2021_mod2)
plot(chewer_2021_mod2)
emmeans(chewer_2021_mod2,specs = pairwise ~ sampling_round) # 3 higher than others
...
emmeans(sucker_2019_mod2,specs = pairwise ~ sampling_round) # 4 higher than others
...
emmeans(sucker_2020_mod2,specs = pairwise ~ sampling_round) # 5 higher than others
...
emmeans(sucker_2021_mod2,specs = pairwise ~ sampling_round) # 3 higher than 1; 4 higher than others

# #singularity check
# summary(sucker_2021_mod2)
# ranef(sucker_2021_mod2) #zeros= bad
# # try running as fixed effect instead of random
# sucker_2021_mod3<-lm(count~seed_coat*as.factor(sampling_round)+variety_name*seed_coat+site,data=filter(sucker,year==2021))
# anova(sucker_2021_mod3) # sampling round and site signif
